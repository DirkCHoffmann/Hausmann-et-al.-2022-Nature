...
function [D, C, S, corrleft, tracesleft, main_analysis, Dir] = functions_ADD(cfile, ifile, pklocs, minspeed, maxspeed, dcutoff, pklim, output, seconds_per_frame, main_analysis, threshcorr, do_null)
%------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
for i=1 : cellnumber
    for ii=1 : cellnumber
        ...
        if not(i) == 0 && not(ii)==0 && D(i,ii) <= dcutoff &&  i~=ii %die anzahl aller Zellpaare unter dcutoff, bei der beide Zellen mindestens 2 peaks haben
            corrleft = corrleft + 1;
        end
    end
end
...
maxshift = ceil(dcutoff/minspeed); % Um die Größe von A und pval schon vor der loop festzuhalten
                ...
                if do_null == 3 % vom zweiten trace wird ein zufällig ausgewählter anderer step ausgewählt
                    for kk = 0 : 1000
                        ...
                        if step_of_other_vector ~= step
                            if step_of_other_vector <= step-(length/steps) || step_of_other_vector >= step+(length/steps)
                                break
                            end
                        end
                    end
